  ...
  return (
    <AdminGuard>
      <div className="flex flex-col flex-1">
        <Header title="Audit Log" subtitle="Tamper-evident record of all significant platform actions" />
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {logs.map((log) => (
                <div key={log.id} className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-sm transition-shadow">
                  <div
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className={cn("text-xs px-2.5 py-1 rounded-full font-medium whitespace-nowrap", ACTION_COLORS[log.action] ?? "bg-gray-100 text-gray-700")}>
                        ...
                        <p className="text-sm text-gray-700">
                          {log.actor ? (
                            <>
                              ...
                              <span className="text-gray-500 text-xs">{log.actor.role}</span>
                            </>
                          ) : (
                  ...
                  {expandedId === log.id && (
                    <div className="border-t border-gray-100 px-4 py-4 space-y-3 bg-gray-50">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Entity Type</p>
                          <p className="text-sm text-gray-700">{log.entityType}</p>
                        </div>
                        <div>
                          <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Entity ID</p>
                          <p className="text-xs text-gray-500 font-mono break-all">{log.entityId}</p>
                        </div>
                        {log.ipAddress && (
                          <div>
                            <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">IP Address</p>
                            <p className="text-sm text-gray-700 font-mono">{log.ipAddress}</p>
                          </div>
                        )}
                        <div>
                          <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Actor Role</p>
                      ...
                      {(log.previousState || log.newState) && (
                        <div className="grid grid-cols-2 gap-4">
                          {log.previousState && (
                            <div>
                              <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">Previous State</p>
                              <pre className="text-xs bg-white border border-gray-200 rounded-lg p-3 overflow-auto">{JSON.stringify(log.previousState, null, 2)}</pre>
                            </div>
                          )}
                          {log.newState && (
                            <div>
                              <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">New State</p>
                              <pre className="text-xs bg-brand-green-lt border border-green-100 rounded-lg p-3 overflow-auto">{JSON.stringify(log.newState, null, 2)}</pre>
                            </div>
                          )}
                        </div>
                      )}
              ...
              {meta && meta.totalPages > 1 && (
                <div className="flex items-center justify-between pt-4">
                  <p className="text-xs text-gray-500">Showing {logs.length} of {meta.total} entries</p>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-500 px-1">{page} / {meta.totalPages}</span>
                    <button
                      onClick={() => setPage(page + 1)}
